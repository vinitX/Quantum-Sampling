...
from cudaq.mlir._mlir_libs._quakeDialects.cudaq_runtime import spin
############################################################
...
#######################################################################
def generate_molecular_spin_ham_unrestricted(h1e,h2e,h2e_prime,ecore):
        ...
        for q in range(nqubits//2):
            ...
            for r in range(nqubits//2):
                for s in range(nqubits//2):
                    ...
                    #two_body_coeff[2*p+1,2*q,2*r,2*s+1]=0.5*h2e[1,p,q,r,s]
...
def jordan_wigner_fermion(h_pq,h_pqrs,ecore,tolerance):
    ...
    #                                    off-diagonal term (excitation operator) h_pq)
    # H_1= sum_pq h_{pq} a_p^dagger a_q + h.c. 
    #
    # For two-body integrals: There are three different terms 
    #                 (diagonal term (coulomb and exchange operator):h_pqqp, h_pqpq, 
    #                   off diagonal terms: (number with excitation operator)h_pqqr, h_pqrp, 
    #                   and (double excitation operator) h_pqrs 
    # H_2 = sum_{pqrs} h_pqrs a_p^dagger a_q^dagger a_r a_s + h.c.
    # 
    ...
    # the anti-commutaion relations of the creation and annihilation operators.
    ...
    for p, q in itertools.combinations(range(nqubit), 2): 
        ...
        #Diagonal term: p=r, q=s or p=s,q=r --> (+ h_pqpq = + h_qpqp = - h_qppq = - h_pqqp)
    ...
    for (p, q), (r, s) in itertools.combinations(itertools.combinations(range(nqubit), 2), 2):

        # h_pqrs = - h_qprs = -h_pqsr = h_qpsr 
...
def jordan_wigner_one_body(p,q,coef):
    ...
    # h_pq(a_p^dagger a_q + a_q^dagger a_p) = R(h_pq) (a_p^dagger a_q + a_q^dagger a_p) + 
    #                                          imag (h_pq) (a_p^dagger a_q - a_q^dagger a_p) 
    # Off-diagonal real part: 0.5 * real(h_{pq}) [ X_p (Z_{p+1}^{q-1}) X_q + Y_p (Z_{p+1}^{q-1}) Y_q ]
    # Off-diagonal imaginary part: 0.5* im(h_pq) [y_p (Z_{p+1}^{q-1}) x_q - x_p (Z_{p+1}^{q-1}) y_q]
        ...
        spin_hamiltonian=0.5*coef.real*spin.x(p)*parity_string*spin.x(q)
        spin_hamiltonian+=0.5*coef.real*spin.y(p)*parity_string* spin.y(q)
        spin_hamiltonian+=0.5*coef.imag*spin.y(p)*parity_string*spin.x(q)
        spin_hamiltonian-=0.5*coef.imag*spin.x(p)*parity_string*spin.y(q)
...
def jordan_wigner_two_body(p,q,r,s,coef):
    ...
    # Diagonal term: p=r, q=s or p=s,q=r --> (+ h_pqpq = + h_qpqp = - h_qppq = - h_pqqp)
    #
    # exchange operator:  h_pqpq (a_p^dagger a_q^dagger a_p a_q) + h_qpqp (a_q^dagger a_p^dagger a_q a_p) 
    # p<q: -1/4 (I_p I_q - I_p Z_q - Z_p I_q+Z_p Z_q)
    #
    # coulomb operator: h_qppq (a_q^dagger a_p^dagger a_p a_q) + h_pqqp (a_p^dagger a_q^dagger a_q a_p) 
    # p<q: 1/4 (I_p I_q - I_p Z_q - Z_p I_q + Z_p Z_q)
    ...
    # + h_pqqs = + h_qpsq = - h_qpqs = - h_pqsq and their hermitian conjugate
    # Real (h_pqqs) (a_p^dagger a_q^dagger a_q a_s + a_s^dagger a_q^dagger a_q a_p) +
    # imag (h_pqqs) (a_p^dagger a_q^dagger a_q a_s - a_s^dagger a_q^dagger a_q a_p)
    # p <q <s: (1/4)(Z_{p+1}^{s-1}) [ I_q {real (h_pqqs/4) (x_p x_s + y_p y_s) + {imag (h_pqqs/4) (y_p x_s - x_p y_s)}
    #                           - Z_q {real (h_pqqs/4) (x_p x_s + y_p y_s) + imag(h_pqqs) (y_p x_s -x_p y_s)}]
    ...
    if len(set([p,q,r,s]))==3:
        ...
        spin_hamiltonian=0.25*coef.real*spin.x(a)*parity_string*spin.x(b)*spin.i(c)
        spin_hamiltonian+=0.25*coef.real*spin.y(a)*parity_string*spin.y(b)*spin.i(c)
        spin_hamiltonian+=0.25*coef.imag*spin.y(a)*parity_string*spin.x(b)*spin.i(c)
        spin_hamiltonian-=0.25*coef.imag*spin.x(a)*parity_string*spin.y(b)*spin.i(c)

        spin_hamiltonian-=0.25*coef.real*spin.x(a)*parity_string*spin.x(b)*spin.z(c)
        spin_hamiltonian-=0.25*coef.real*spin.y(a)*parity_string*spin.y(b)*spin.z(c)
        spin_hamiltonian-=0.25*coef.imag*spin.y(a)*parity_string*spin.x(b)*spin.z(c)
        spin_hamiltonian+=0.25*coef.imag*spin.x(a)*parity_string*spin.y(b)*spin.z(c)


    # Off-diagonal term with four different sets of non-equal indices
    # h_pqrs = h_qpsr = - h_qprs = - h_pqsr
    # real {h_pqrs} (a_p^dagger a_q^dagger a_r a_s + a_s^dagger a_r^dagger a_q a_p) + 
    # imag (h_pqrs) (a_p^dagger a_q^dagger a_r a_s - a_s^dagger a_r^dagger a_q a_p)
    ...
    # also we need to compute p<r<q<s and p<r<s<q
        ...
        if p<q<r<s:
            ...
            spin_hamiltonian =-0.125*coef.real*spin.x(a)*parity_string_a*spin.x(b)*spin.x(c)*parity_string_b*spin.x(d)
            spin_hamiltonian-=-0.125*coef.real*spin.x(a)*parity_string_a*spin.x(b)*spin.y(c)*parity_string_b*spin.y(d)
            spin_hamiltonian+=-0.125*coef.real*spin.x(a)*parity_string_a*spin.y(b)*spin.x(c)*parity_string_b*spin.y(d)
            spin_hamiltonian+=-0.125*coef.real*spin.x(a)*parity_string_a*spin.y(b)*spin.y(c)*parity_string_b*spin.x(d)
            spin_hamiltonian+=-0.125*coef.real*spin.y(a)*parity_string_a*spin.x(b)*spin.x(c)*parity_string_b*spin.y(d)
            spin_hamiltonian+=-0.125*coef.real*spin.y(a)*parity_string_a*spin.x(b)*spin.y(c)*parity_string_b*spin.x(d)
            spin_hamiltonian-=-0.125*coef.real*spin.y(a)*parity_string_a*spin.y(b)*spin.x(c)*parity_string_b*spin.x(d)
            spin_hamiltonian+=-0.125*coef.real*spin.y(a)*parity_string_a*spin.y(b)*spin.y(c)*parity_string_b*spin.y(d)

            spin_hamiltonian+=0.125*coef.imag*spin.x(a)*parity_string_a*spin.x(b)*spin.x(c)*parity_string_b*spin.y(d)
            spin_hamiltonian+=0.125*coef.imag*spin.x(a)*parity_string_a*spin.x(b)*spin.y(c)*parity_string_b*spin.x(d)
            spin_hamiltonian-=0.125*coef.imag*spin.x(a)*parity_string_a*spin.y(b)*spin.x(c)*parity_string_b*spin.x(d)
            spin_hamiltonian+=0.125*coef.imag*spin.x(a)*parity_string_a*spin.y(b)*spin.y(c)*parity_string_b*spin.y(d)
            spin_hamiltonian-=0.125*coef.imag*spin.y(a)*parity_string_a*spin.x(b)*spin.x(c)*parity_string_b*spin.x(d)
            spin_hamiltonian+=0.125*coef.imag*spin.y(a)*parity_string_a*spin.x(b)*spin.y(c)*parity_string_b*spin.y(d)
            spin_hamiltonian-=0.125*coef.imag*spin.y(a)*parity_string_a*spin.y(b)*spin.x(c)*parity_string_b*spin.y(d)
            spin_hamiltonian-=0.125*coef.imag*spin.y(a)*parity_string_a*spin.y(b)*spin.y(c)*parity_string_b*spin.x(d)
